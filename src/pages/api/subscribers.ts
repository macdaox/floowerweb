...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!await consumeRateLimit(createDb(db), `subscribers:${clientIp(request)}`, LIMIT, WINDOW_SECONDS)) {
      return errorResponse(new HttpError("too_many_requests", "Too many subscription attempts. Please try again later.", 429), native);
    }
...
function responseFor(request: Request, native: boolean, source: string, json: Response): Response {
  if (!native) return json;
  const destination = new URL(source, request.url);
  destination.searchParams.set("submitted", "subscriber");
  if (destination.pathname === "/") destination.hash = "footer";
