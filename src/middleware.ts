...
import { findSession, readSessionCookie, renewSessionIfNeeded, SESSION_COOKIE, sessionCookie } from "./features/auth/session";
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  if (token) {
    ...
    if (session) {
      ...
      if (!clearsSessionCookie(response)) {
  ...
  if (path === "/api/admin" || path.startsWith("/api/admin/")) return fail("unauthorized", "Authentication is required.", 401);
...
function clearsSessionCookie(response: Response): boolean {
  return response.headers.get("set-cookie")?.includes(`${SESSION_COOKIE}=;`) ?? false;
}
