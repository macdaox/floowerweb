...
export function initializeSubscriberList(root: HTMLElement): void {
  ...
  root.replaceChildren(toolbar, feedback, content);
  for (const control of [status, from, to]) control.addEventListener("change", () => void load());
  for (const control of [source, search]) control.addEventListener("input", debounce(() => void load()));
  void load();
  ...
  async function load(): Promise<void> {
    ...
    try {
      ...
      const body = await response.json() as { ok: boolean; data?: { items: Subscriber[]; total: number }; error?: { message?: string } };
