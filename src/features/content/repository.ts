...
export async function listPublishedSpaceRows(db: D1Database, locale: string): Promise<Row[]> {
  const result = await db.prepare(`SELECT s.id, s.title, s.slug, s.category, s.location, s.summary, s.body, s.seo_title, s.seo_description, m.object_key, m.original_filename, m.width, m.height,
    COALESCE((SELECT si.alt_text FROM space_images si WHERE si.space_id = s.id AND si.media_id = s.cover_media_id ORDER BY si.sort_order, si.id LIMIT 1), m.alt_text) AS alt_text
...
export async function findPublishedSpaceRow(db: D1Database, locale: string, slug: string): Promise<Row | null> {
  return db.prepare(`SELECT s.id, s.title, s.slug, s.category, s.location, s.summary, s.body, s.seo_title, s.seo_description, m.object_key, m.original_filename, m.width, m.height,
    COALESCE((SELECT si.alt_text FROM space_images si WHERE si.space_id = s.id AND si.media_id = s.cover_media_id ORDER BY si.sort_order, si.id LIMIT 1), m.alt_text) AS alt_text
    FROM spaces s LEFT JOIN media m ON m.id = s.cover_media_id AND m.is_deleted = 0
...
export async function listSpaceImageRows(db: D1Database, spaceId: string): Promise<Row[]> {
  const result = await db.prepare(`SELECT m.object_key, m.original_filename, m.width, m.height, COALESCE(si.alt_text, m.alt_text) AS alt_text FROM space_images si
...
export async function listRelatedSpaceRows(db: D1Database, locale: string, category: string, id: string): Promise<Row[]> {
  const result = await db.prepare(`SELECT s.title, s.slug, s.category, s.location, s.summary, m.object_key, m.original_filename, m.width, m.height,
    COALESCE((SELECT si.alt_text FROM space_images si WHERE si.space_id = s.id AND si.media_id = s.cover_media_id ORDER BY si.sort_order, si.id LIMIT 1), m.alt_text) AS alt_text FROM spaces s
...
export async function listPublishedArticleRows(db: D1Database, locale: string): Promise<Row[]> {
  const result = await db.prepare(`SELECT a.id, a.title, a.slug, a.summary, a.body, a.author, a.published_at, a.seo_title, a.seo_description, m.object_key, m.original_filename, m.width, m.height, m.alt_text
...
export async function findPublishedArticleRow(db: D1Database, locale: string, slug: string): Promise<Row | null> {
  return db.prepare(`SELECT a.id, a.title, a.slug, a.summary, a.body, a.author, a.published_at, a.seo_title, a.seo_description, m.object_key, m.original_filename, m.width, m.height, m.alt_text
...
export async function listRelatedArticleRows(db: D1Database, locale: string, id: string): Promise<Row[]> {
  const result = await db.prepare(`SELECT a.title, a.slug, a.summary, a.author, a.published_at, m.object_key, m.original_filename, m.width, m.height, m.alt_text FROM articles a
